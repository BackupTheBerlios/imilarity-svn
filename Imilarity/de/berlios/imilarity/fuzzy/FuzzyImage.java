...
public class FuzzyImage extends FuzzySetBase {
	...
	public FuzzySet intersection(FuzzySet set) {
		...
		ArrayFuzzySet result = new ArrayFuzzySet(pc);
		for (int i = 0; i < pc; i++) {
			Membership m = getMembership(i);
			result.addMembership(i, new ColorMembership(m.and(set.getMembership(i)).getComponents()));
		}
		return result;
	}
	...
	public FuzzySet union(FuzzySet set) {
		...
		for (int i = 0; i < pc; i++) {
			Membership m = getMembership(i);
			result.addMembership(i, new ColorMembership(m.or(set.getMembership(i)).getComponents()));
		}
		return result;
	}
	...
	public FuzzySet complement() {
		...
		for (int i = 0; i < pc; i++) {
			Membership m = getMembership(i);
			result.addMembership(i, new ColorMembership(m.complement().getComponents()));
		}
		return result;
	}
