...
\cite{ohta:color_info_for_region_segm}, is een \defin{opponent color space}. 
...
reduceren. Dat kan door middel van \defin{kleurkwantisatie}. Daarbij groeperen we de kleuren in 
...
%\vspace{2pt}
\caption{\label{fig:probleem_focale_kwantisatie}Onverwachte mapping bij focale kwantisatie.}
\end{figure}
...
Het \defin{binomiaalfilter}{3x3 binomiaalfilter} is een voorbeeld van een lineair filter. Voor het filtermasker 
...
Beeldranden in een tweedimensionaal beeld $b$ zijn plaatsen waar de luminantiecomponent $lum(x,y)$ van $b(x,y)$ 
sterk varieert als functie van $x$ en/of $y$. Op die plaatsen zullen de partieel afgeleiden $D_1 lum(x,y)$ en 
$D_2 lum(x,y)$ dus groot zijn. Bijgevolg zal de \defin{gradi\"ent} $\nabla lum$ daar ook groot zijn. We kunnen 
...
\end{displaymath}
De \defin{Sobel-operator} is een variant hierop die minder ruisgevoelig is doordat er 
