...
$$
met $A(x)=(A_1(x),A_2(x),A_3(x))$ en $B(x)=(B_1(x),B_2(x),B_3(x))$ voor elke $x$ uit het universum. Als we
...
twee vaagverzamelingen in een zelfde universum $X$ zijn, waarvoor $A(x) \le B(x)$ voor alle
$x \in X$. Er geldt dan $A \cap B$ = A en ook $\min\{|A|,|B|\}=|A|$ zodat
$$
M_8(A,B)= \frac{|A \cap B|}{\min\{|A|,|B|\}} = \frac{|A|}{|A|} = 1
$$
als $|A| \ne 0$. Hoewel $A(x) \le B(x)$ in de praktijk zelden zal gelden voor alle $x$ uit $X$, 
is het niet uitzonderlijk dat die ongelijkheid geldt voor de meeste $x \in X$. In dergelijke 
