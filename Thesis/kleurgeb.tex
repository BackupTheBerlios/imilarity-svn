...
% & \displaystyle \sum_{c' \in C'} \left( \begin{array}{c} \displaystyle \sum_{p \in P} \delta (bin(c')-bin(A(p))) \end{array} \right) \cdot \delta (bin(c)-bin(c')) \\[2pt]
...
Zij $g$ de restrictie van $h_A \circ kleur$ tot
...
\caption{\label{fig:uitgebreide_sct-histogrammen}De pseudo-vage SCT-histogrammen (b) voor de 
beelden van het geel bootje (a), samen met de corresponderende pseudo-vage (c) gladde histogrammen, 
(d) spatiale histogrammen, (e) zowel gladde als
spatiale histogrammen, (f) randhistogrammen en (g) gladde randhistogrammen.}
\end{center}
\end{figure}
