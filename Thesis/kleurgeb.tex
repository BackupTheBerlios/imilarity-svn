...
histogrammen} te beschouwen. De uitgebreide versie $\mathcal{H}'_A$ van een histogram
$\mathcal{H}_A \in \{H_A,\widetilde{H}_A\}$ geassocieerd met een beeld $A$ defini\"eren we
als volgt:
$$
\mathcal{H}'_A(c) = \sum_{k \in \Omega_{s_A}} \mathcal{H}_A(kleur(bin(c)+k))
\cdot w_{A,c}(k) $$ 
voor alle $c \in C'$, met $w_{A,c}$ een $\mathbb{N} - \mathbb{R}$ afbeelding en 
$\Omega_{s_{A,c}}=\{k \in \mathbb{N} \mid s_{A,c}(k) \ne 0\}$. Indien $s_{A,c}(k)=\delta(k)$,
...
te herleiden tot het eerste voorstel van Swain en Ballard. Dat voorstel komt
...
s_{A,c}(k)=
\left\{ \begin{array}{ll}
\alpha^{-|k|} & \textrm{als } bin(c) \le 16 \textrm{ en } |k| \le R_1 \\
\beta^{-|k|} & \textrm{als } bin(c) > 16 \textrm{ en } |k| \le R_2 \\ 
0 & \textrm{anders}
\end{array}
$$
voor alle $(A,c,k)$ uit $\mathcal{F}_L(P) \times C' \times \mathbb{N}$, met
$\alpha$, $\beta$, $R_1$ en $R_2$ vrij te kiezen parameters.
