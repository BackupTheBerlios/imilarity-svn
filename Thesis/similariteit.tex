...
Stel dat $A$ en $B$ twee (L-)vaagverzamelingen zijn in een zelfde universum $X$. Het kan dan voorkomen dat 
...
% \end{displaymath}
Wanneer we voor elke $C$ uit $\mathcal{F}_L(X)$
\begin{displaymath}
||C|| = \sum_{x \in X'} C(x)
\end{displaymath} 
...
\end{displaymath}
voor alle $C \in \mathcal{F}_L(X)$, vinden we bovendien:
\begin{displaymath}
\begin{array}{l@{\ =\ }l@{\ =\ }l@{\ =\ }l}
