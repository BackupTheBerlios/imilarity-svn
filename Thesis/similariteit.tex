...
| A \cap B | & \displaystyle \sum_{x \in X} \min \{A(x),B(x)\} & \\
& \displaystyle \sum_{x \in X'} \min \{A(x),B(x)\} & (\min\{0,0\}=\min\{0\}=0) \\
& ||A \cap B|| & \\[5pt]
| A \cup B | & \displaystyle \sum_{x \in X} \max \{A(x),B(x)\} & \\
& \displaystyle \sum_{x \in X'} \max \{A(x),B(x)\} & (\max\{0,0\}=\max\{0\}=0) \\
& ||A \cup B|| & \\[5pt]
| A \setminus B | & | A \cap B^c | & \\
& \displaystyle \sum_{x \in X} \min \{A(x),1-B(x)\} & \\
& \displaystyle \sum_{x \in X'} \min \{A(x),1-B(x)\} & (\min\{0,1-0\}=\min\{0,1\}=0) \\
& || A \cap B^c || & \\
& ||A \setminus B|| & \\[5pt]
| A \triangle B | & | (A \setminus B) \cup (B \setminus A) | & \\
& || (A \setminus B) \cup (B \setminus A) || & (\min\{0,1\}=0 \textrm{ en } \max\{0,0\}=1) \\
& || A \triangle B || & \\
\end{array}
$$
$$
\begin{array}{r@{\quad = \quad}l@{\qquad}l}
|(A \cap B) \cap (A^c \cap B^c)| & ||(A \cap B) \cap (A^c \cap B^c)|| & (\min\{0,1\}=0)\\[5pt]
|(A \cup B) \cap (A^c \cup B^c)| & ||(A \cup B) \cap (A^c \cup B^c)|| & (\min\{0,1\}=0)
\end{array}
$$
...
Beschouw bijvoorbeeld $M_{5c}$. We herschrijven de oorspronkelijke formule als volgt:
$$
\begin{array}{rclcl}
...
\end{array}
$$
Bij de oude formule moeten er twee keer twee getallen met elkaar vergeleken worden en zijn er
$4 \cdot (|X|-1)$ optellingen en \'e\'en deling nodig. De herschreven vorm vereist twee aftrekkingen, 
twee vergelijkingen, $ 4 \cdot (|X'|-1)$ optellingen en \'e\'en deling. Vermits $|X'| << |X|$ impliceert 
dat $4 \cdot (|X'|-1) + 5 << 4 \cdot (|X|-1) + 3$, zal de nieuwe formule dus inderdaad een stuk sneller kunnen 
berekend worden dan de oude als $X'$ veel minder elementen bevat dan $X$. Bovendien is het verschil in 
rekentijd verwaarloosbaar als $X'=X$. De nieuwe vorm is dus steeds minstens even snel als de oude.